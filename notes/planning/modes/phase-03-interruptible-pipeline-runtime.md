...
[ ] 3 Phase 3 - Mode Orchestration Runtime Foundation (`jido_code_server`)
  ...
  [ ] 3.2 Section - Mode Registry and Configuration in Code Server
    ...
    [ ] 3.2.4 Task - Define action-backed tool declaration and LLM exposure contract in code server
      ...
      [ ] 3.2.4.1 Subtask - Define `ToolCatalog` ownership for built-in, provider-registered action tools, and template-derived tools.
      [ ] 3.2.4.2 Subtask - Define provider adapter contracts for `jido_command` and `jido_workflow` so tool specs come from runtime action contracts (`name/description/schema`) rather than raw markdown assets.
      [ ] 3.2.4.3 Subtask - Define deterministic tool-exposure filters per conversation/mode before LLM execution.
      [ ] 3.2.4.4 Subtask - Define validation/rejection behavior for unknown, non-exposed, or invalidly-registered provider tools.
  ...
  [ ] 3.4 Section - Phase 3 Integration Tests
    ...
    [ ] 3.4.1 Task - Mode state and config integration scenarios
      ...
      [ ] 3.4.1.5 Subtask - Verify only declared/exposed action-backed `ToolCatalog` tools are surfaced to LLM strategy/tool loops.
      [ ] 3.4.1.6 Subtask - Verify raw `.jido` markdown presence does not expose LLM tools without successful provider registration.
