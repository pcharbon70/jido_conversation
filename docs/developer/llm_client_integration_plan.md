...
- Extend effect-manager harness cancellation parity coverage to invalid
  explicit `cause_id` fallback for successful backend cancel outcomes.
- Ensure harness cancel-ok invalid-cause flows preserve deterministic uncoupled
  trace fallback semantics, terminal lifecycle exclusivity, and cancel
  telemetry invariants.

### Tasks

- Add effect-manager harness cancel-ok invalid `cause_id` test to assert:
  - request dispatch resolves to `backend: :harness`
  - backend cancel callback is invoked with active execution reference
  - canceled lifecycle payload includes:
    - `reason: "user_abort"`
...
- Hardened effect-manager cancel-ok invalid-cause fallback coverage for
  `harness` backend.

### Exit criteria

- Effect-manager runtime tests verify deterministic harness cancel-ok
  invalid-cause fallback semantics with expected uncoupled trace behavior,
  attribution/telemetry, and no terminal-state regressions.
...
- Extend effect-manager harness cancellation parity coverage to explicit
  `cause_id` linkage for missing-execution-reference cancel outcomes.
- Ensure harness cancel-not-available cause-linked flows preserve deterministic
  trace linkage semantics, terminal lifecycle exclusivity, and cancel telemetry
  invariants.

### Tasks

- Add effect-manager harness cancel-not-available explicit `cause_id` test to
  assert:
  - request dispatch resolves to `backend: :harness`
...
- Hardened effect-manager cancel-not-available cause-link coverage for
  `harness` backend.

### Exit criteria

- Effect-manager runtime tests verify deterministic harness
  cancel-not-available cause-link semantics with expected trace linkage/
  attribution/telemetry and no terminal-state regressions.
...
- Extend effect-manager harness cancellation parity coverage to explicit
  `cause_id` linkage for backend-failed cancel outcomes.
- Ensure harness cancel-failed cause-linked flows preserve deterministic trace
  linkage semantics, terminal lifecycle exclusivity, and cancel telemetry
  invariants.

### Tasks

- Add effect-manager harness cancel-failed explicit `cause_id` test to assert:
  - request dispatch resolves to `backend: :harness`
  - backend cancel callback is invoked with active execution reference and
    failed cancel scenario
  - canceled lifecycle payload includes:
    - `reason: "user_abort"`
    - `backend_cancel: "failed"`
    - cancel error metadata (`reason`, `category`, `retryable?`)
    - `backend: "harness"`
    - backend/provider/model attribution
  - backward trace chain includes:
    - canceled lifecycle signal id
...
- Hardened effect-manager cancel-failed cause-link coverage for `harness`
  backend.

### Exit criteria

- Effect-manager runtime tests verify deterministic harness
  cancel-failed cause-link semantics with expected trace linkage/attribution/
  telemetry and no terminal-state regressions.
...
- Extend effect-manager harness cancellation parity coverage to invalid
  explicit `cause_id` fallback for backend-failed cancel outcomes.
- Ensure harness cancel-failed invalid-cause flows preserve deterministic
  uncoupled trace fallback semantics, terminal lifecycle exclusivity, and
  cancel telemetry invariants.

### Tasks

- Add effect-manager harness cancel-failed invalid `cause_id` test to assert:
  - request dispatch resolves to `backend: :harness`
  - backend cancel callback is invoked with active execution reference and
    failed cancel scenario
  - canceled lifecycle payload includes:
...
- Hardened effect-manager cancel-failed invalid-cause fallback coverage for
  `harness` backend.

### Exit criteria

- Effect-manager runtime tests verify deterministic harness
  cancel-failed invalid-cause fallback semantics with expected uncoupled trace
  behavior, attribution/telemetry, and no terminal-state regressions.
