...
| Phase 47 | `completed` | Effect-manager LLM cancel-failed invalid-cause fallback parity hardening | Invalid cancel `cause_id` uncoupled tracing and failed-cancel telemetry invariants in effect runtime tests |
...
- Harden effect runtime LLM cancellation parity coverage for invalid explicit
  `cause_id` values when backend cancellation fails.
- Ensure canceled lifecycle ingestion falls back to uncoupled trace linkage
...
- Hardened effect manager cancellation test assertions for failed-cancel invalid
  cause fallback tracing and cancel telemetry invariants.
