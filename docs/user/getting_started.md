...
{:ok, _result} =
  Jido.Conversation.Ingest.ingest(%{
...
```elixir
timeline = Jido.Conversation.Projections.timeline(conversation_id)
llm_context = Jido.Conversation.Projections.llm_context(conversation_id)
```
...
metrics = Jido.Conversation.Telemetry.snapshot()
```
